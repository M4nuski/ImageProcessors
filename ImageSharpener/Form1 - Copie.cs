    ...
    public partial class Form1 : Form
    {
      ...
      //  public int ThreadProgress;
        ...
        private void SmoothPixels(int smoothRange)
        {
            ...
            if (comboBox1.SelectedIndex == 3) // Square Linear
            {
                for (x = smoothRange; x >= 0 ; x--)
                {
                    for (k = -smoothRange; k < smoothRange + 1; k++)
                    {
                        for (l = -smoothRange; l < smoothRange + 1; l++)
                        {
                            if (x > Math.Abs(k) & x > Math.Abs(l))
                                d = 1.0000d/ (smoothRange + 1); else d = 0;

                            Distances[k + smoothRange, l + smoothRange] += d;
                            MaxStack += d;
                        }
                    }
                }
            ...
            if (comboBox1.SelectedIndex == 2) // Square Cross
            {
                for (k = -smoothRange; k < smoothRange + 1; k++)
                {
                    for (l = -smoothRange; l < smoothRange + 1; l++)
                    {
                        d = (0.5d * (smoothRange-Math.Abs(k))) + (0.5d * (smoothRange-Math.Abs(l)));
            ...
            if (comboBox1.SelectedIndex == 3) // Round Full
            {
                for (k = -smoothRange; k < smoothRange + 1; k++)
                {
                    for (l = -smoothRange; l < smoothRange + 1; l++)
                    {
                        d = Math.Sqrt((l * l) + (k * k));
                        d = (d <= smoothRange) ? 1.0000d : 0.0000d;
            ...
            if (comboBox1.SelectedIndex == 4) // Round Linear
            {
                for (k = -smoothRange; k < smoothRange + 1; k++)
                {
                    for (l = -smoothRange; l < smoothRange + 1; l++)
                    {
                        d = Math.Sqrt((l*l) + (k*k));
                        d = Clamp(1.0000d - (d / smoothRange), 0.0000d, 1.0000d);
                        Distances[k + smoothRange, l + smoothRange] = d;
                        MaxStack += d;
                        if (d > maxSingle) maxSingle = d;
                    }
            ...
            if (comboBox1.SelectedIndex == 5) // Round Spherical
            {
                for (k = -smoothRange; k < smoothRange + 1; k++)
                {
                    for (l = -smoothRange; l < smoothRange + 1; l++)
            ...
            var bmp = new Bitmap(2 * smoothRange + 2, 2 * smoothRange + 2);

            for (k = -smoothRange; k < smoothRange + 1; k++)
            {
                for (l = -smoothRange; l < smoothRange + 1; l++)
                {
                    bmp.SetPixel(1+k + smoothRange, 1+l + smoothRange, Color.FromArgb((int)(255 * Distances[k+smoothRange, l+smoothRange]/maxSingle),Color.White));
                }
            }

            for (k = -smoothRange; k < smoothRange + 1; k++)
            {
                bmp.SetPixel(1 + k + smoothRange, smoothRange - (int)((smoothRange)*(Distances[k + smoothRange, smoothRange / 2 + 1] / maxSingle)), Color.Red);
            }
                ...
                for (y = 0; y < _height - 1; y++)
                {
                    ...
                    for (smoothingX = -smoothRange; smoothingX < smoothRange + 1; smoothingX++)
                    {
                        for (smoothingY = -smoothRange; smoothingY < smoothRange + 1; smoothingY++)
                        {
                            ...
                            SmoothBuffer[x, y, channel] += (PixelSource[offset] * Distances[smoothingX + smoothRange, smoothingY + smoothRange]);
                        }
                    }
        ...
        private void SmoothChannelHV(int channel, int smoothRange)
        {
                   ...
                   // {
                        for (smoothingY = -smoothRange; smoothingY < smoothRange + 1; smoothingY++)
                        {
                          ...
                          //  if (computedX > _width - 1) computedX = _width - 1;
                            ...
                            SmoothBuffer[x, y, channel] += (PixelSource[offset] * Distances[smoothRange, smoothingY + smoothRange]);
                        }
                   // }
              //      SmoothBuffer[x, y, channel] /= MaxStack;
                }
            ...
            for (x = 0; x < _width - 1; x++)
            {
                for (y = 0; y < _height - 1; y++)
                {
                 ...
                 //   {
                        ...
                        SmoothBuffer[x, y, channel] += (PixelSource[offset] * Distances[smoothRange, smoothingX + smoothRange]);
                    }
                    // }
                //    SmoothBuffer[x, y, channel] /= MaxStack;
                }
           ...
           double mshv = 0;
                for (smoothingY = -smoothRange; smoothingY < smoothRange + 1; smoothingY++)
                {
                    mshv += Distances[smoothRange, smoothingY + smoothRange];
                }
                for (smoothingY = -smoothRange; smoothingY < smoothRange + 1; smoothingY++)
                {
                    mshv += Distances[smoothRange, smoothingY + smoothRange];
